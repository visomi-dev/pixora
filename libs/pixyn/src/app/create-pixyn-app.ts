...
import type { ApplicationContext, PixynApp, PixynAppOptions } from './types';

type ServiceRegistryWithContext = ServiceRegistry & {
  setContext(context: ApplicationContext): void;
};

export async function createPixynApp(options: PixynAppOptions): Promise<PixynApp> {
  ...
  const pixynApp: PixynApp = {
    ...
    async start() {
      if (destroyed) {
        throw new Error('Pixyn app has already been destroyed.');
      }
  ...
  if (options.autoStart !== false) {
    await pixynApp.start();
  }

  return pixynApp;
}

function validateOptions(options: PixynAppOptions): void {
  if (!options.mount) {
    throw new Error('Pixyn requires a mount element.');
  }

  if (options.scenes.length === 0) {
    throw new Error('Pixyn requires at least one scene definition.');
  }
